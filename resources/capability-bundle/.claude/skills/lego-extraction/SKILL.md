...
Each candidate must separate semantic core, logical skeleton, and language shell. Add retrieval cues, slots, connectors, evidence links, scope, and counterexamples as described in [references/module-schema.md](references/module-schema.md). Copy [scripts/validate-candidate.mjs](scripts/validate-candidate.mjs) into `scratch/` to validate new formats or extend checks.

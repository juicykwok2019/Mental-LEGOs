...
const runtimeManifestSchema = z.object({
  ...
  provider_proxy: z.object({
    file: z.literal('MentalLegos.ProviderProxy.exe'),
    bytes: z.number().int().positive(),
    sha256: z.string().regex(/^[a-f0-9]{64}$/u),
  }),
});
...
export async function verifyProviderProxy(
  executablePath: string,
  manifestPath: string,
): Promise<{ sha256: string }> {
  const manifest = runtimeManifestSchema.parse(
  ...
  if (actual.bytes !== manifest.provider_proxy.bytes
    || actual.sha256 !== manifest.provider_proxy.sha256) {
    throw new Error('Provider proxy integrity verification failed.');
  }
  return { sha256: actual.sha256 };
}
